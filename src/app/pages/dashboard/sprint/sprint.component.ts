...
import { Issue, Task ,RelevantData} from '../../../user.interface';
...
filteredData:RelevantData[]=[]
filteredData2:string[]=[]

relevantDataEntries:[string, any][]=[]
constructor(private dialog:MatDialog,private localStorageService: StorageService){}
ngOnInit(): void {
  let getdata:Issue=this.localStorageService.getItem('Issue')
  // let storedData= getdata ? JSON.parse(getdata) : [];
  // console.log(typeof storedData)
  
  if(Array.isArray(getdata)){
    
    this.filteredData=getdata.map((name)=>({
      summary:name.summary,
      status:name.status

    }))
    console.log(this.filteredData)
    // console.log(filteredData,"summary")
  }
...
openDialog3(): void {
  ...
  const dialogRef = this.dialog.open(EditBacklogComponent, dialogConfig);
